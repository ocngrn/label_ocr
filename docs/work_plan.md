...
**정식 수치** (2026-08-05 재측정, test 234장 / GT 762박스, 누수 없음)

| 가중치 | 해상도 | plane recall | precision | Hmean | 예측수 |
|---|---|---|---|---|---|
| 사전학습 | 960/max | 68.7% | 59.3% | 63.4% | 875 |
| 사전학습 | 원본 2560×1920 | 75.4% | 60.1% | 66.6% | 949 |
| 학습후 | 960/max | 74.4% | **82.3%** | **77.7%** | **682** |
| **학습후** | **원본 2560×1920** | **78.9%** | 62.5% | 69.5% | 954 |

- **해상도와 fine-tuning 은 독립 효과**, 각각 +3.5~6.7%p, 합계 **+10.2%p**
- precision 59.3% → 82.3% (+23.0%p), 예측 875 → 682 (GT 762) 가 **박스 규약 학습의 증거**
...
- 최초 보고는 곡면 증강(train 75장 유입)으로 오염돼 있었다. 편향은 작았으나(+0.4%p)
  **곡면 수치는 폐기.** 코드는 `extra_curved=False` 기본으로 고쳤다

---

### Phase 9 — DB 유도 조각 병합 — ✅ **작동 확인 (2026-08-05)**

근거 → [`reports/db_merge.md`](../reports/db_merge.md) / 코드 `src/eval/db_merge.py`

| 방식 | 시스템 Top-5 | tok1 | tok2 | tok3 |
|---|---|---|---|---|
| 기하 `gap=0.0` (M2-B 최적) | 62.9% | 67% | 46% | 57% |
| 기하 `gap=1.5` | 40.3% | 32% | 62% | 67% |
| **DB 유도 (무조정)** | **64.2%** | 64% | **61%** | **68%** |

- M2-B 가 "기하로는 원리적으로 불가능"이라 결론낸 구분을 **DB 가 해준다**.
  `8196P32` 는 DB 에 있고 `P32C6B` 는 없다 — 이미지에 없는 정보가 부품 마스터에는 있다
- 다중 토큰 tok2 **+15%p**, tok3 **+11%p**. 같은 이득을 기하로 얻으려면 `gap=1.5` 가 필요한데
  그러면 단일 토큰이 67%→32% 로 무너진다. **DB 유도는 단일 토큰 손실이 3%p 뿐**
- 라벨 관행 실측: 527개 토큰(13.3%)이 단독으로도 묶인 채로도 등장, 등장 횟수 2914 : 2814.
  **같은 토큰이 사진마다 다르게 라벨링된다** — 판단 근거는 토큰이 아니라 문맥이다
- ⚠️ `margin=0.02` 로 65.9% 가 나오지만 **test 에서 고른 값**이라 주장 불가.
  그리고 이 실험은 **PP-OCRv5 캐시** 기반이다 (아래 다음 행동에서 해소)

---

## 🔴 현재 위치 및 다음 행동

**시스템 Top-5 = 67.5% 추정** (plane recall 78.9% × 조건부 인식 85.6%) / M3 목표 81% **미달**

> ✅ **누수 없는 정식 수치** (2026-08-05 재측정, test 234장 / GT 762박스만).
> 사전학습 64.6% → 학습후 67.5%. 상세 → [`reports/m3_det_finetune.md`](../reports/m3_det_finetune.md)

### 저비용 경로가 이번에도 전부 소진됐다

| 시도 | 결과 |
|---|---|
| 검출 fine-tuning | ✅ plane recall +3.5%p (원본 해상도), precision +23.0%p (960/max) |
| `limit_side_len` 상향 | ✅ 원본 해상도가 최선. +6.7%p (사전학습 기준) |
| **후처리 재스윕** | ❌ **개선 없음** — 기본값(0.3/0.6/1.5)이 이미 최적 |
| 더 오래 학습 | ❌ best 는 epoch 24, 이후 26 epoch 갱신 없음 |

### ⚠️ 두 수치를 곱하거나 비교하지 말 것

| 수치 | 무엇인가 | 검출기 |
|---|---|---|
| **67.5%** | 검출만 재고 `plane recall × 85.6%` 로 **추정** | fine-tuned v4 |
| **64.2%** | 검출→인식→DB 매칭 **전 구간 실측** | PP-OCRv5 (구) |

축이 다르다. 67.5% 의 두 번째 항 85.6%는 **PP-OCRv5 박스로 잰 낡은 값**이고,
64.2% 는 fine-tuned 검출기를 아직 안 쓴 값이다. **둘을 합치려면 새 캐시가 필요하다.**

---

### 🎯 다음 단 하나의 행동 — Colab 세션 1회 (약 40분)

**사용자 조치**: 노트북 `notebooks/train_det.ipynb` 의 **셀 1 → 2 → 2-T** 실행 후
출력된 **터널 주소 한 줄**을 전달. 그 뒤는 전부 SSH 로 처리한다.

| # | 작업 | 명령 | 시간 |
|---|---|---|---|
| **A** | 추론 해상도 **확대** 측정 | `detect_trained --limit-side-len 2880 --limit-type min` (와 3840) | 10분 |
| **B** | fine-tuned 검출기로 **val + test 캐시** 재생성 | `spatial_group.build_cache` 를 새 검출기로 | 20분 |
| **C** | 그 캐시로 조건부 인식률 재측정 + `margin` 을 **val 에서** 확정 | `db_merge` | 10분 |

**B 의 캐시 하나가 세 가지를 동시에 푼다** — ①67.5% 추정의 낡은 두 번째 항,
②`margin` 을 val 에서 골라야 하는 문제, ③새 검출기에서 DB 병합이 여전히 유효한지.

> **A 를 먼저 하는 이유**: `limit_type='min'` 은 짧은 변이 기준값보다 **작을 때만 확대**한다.
> 우리 이미지는 2560×1920 이라 736/1120/1600 은 전부 `ratio=1.0` 이었다(무효).
> **원본보다 큰 해상도는 한 번도 시험한 적이 없다.** 학습 때 글자를 0.5~3배로 보여줬으므로
> 추론이 학습 분포의 아래쪽 끝에 있을 가능성이 있다.

### 그 다음 (A·B·C 결과에 따라 갈림)

1. **학습/추론 스케일 정합** — A 가 효과 없으면 재학습으로. 학습 1회(~2시간)
2. **M4 인식 fine-tuning** — 인식은 아직 한 번도 손대지 않았다 (`metrics_policy.md` §5-B)
3. 학습 데이터 증대 — 가장 확실하지만 라벨링 비용

**C 에서 조건부 인식률이 떨어졌다면 2번이 최우선이다** (검출 개선이 인식에서 상쇄된다는 뜻).

---

### Colab 재접속 절차 (세션이 바뀌어도 이대로)

```bash
# 1) 터널 클라이언트 (세션마다 1회, 수 초)
curl -fsSL -o scripts/cloudflared.exe \
  https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-windows-amd64.exe

# 2) 사용자가 노트북 셀 1 → 2 → 2-T 실행 → 터널 주소 획득

# 3) 코드 전송 (PAT 불필요 — 로컬 작업본이 진실)
sh scripts/colab_push_code.sh <터널주소>

# 4) 원격 실행
sh scripts/colab_ssh.sh <터널주소> 'nvidia-smi'
sh scripts/colab_ssh.sh <터널주소> '[ -d /content/PaddleOCR ] || git clone -q --depth 1 --branch v2.10.0 https://github.com/PaddlePaddle/PaddleOCR.git /content/PaddleOCR'
```

- **키**: `~/.ssh/colab_label_ocr` (공개키는 노트북 셀 2-T 에 박혀 있음). 없으면 새로 만들고
  셀 2-T 의 `PUBKEY` 를 교체 — 붙여넣을 때 **줄바꿈이 문자열에 들어가면 SyntaxError** 로 죽는다
- **`pkill -f tools/train.py` 금지** — 래퍼의 `bash -c` 명령줄에도 그 문자열이 있어
  **자기 세션을 죽인다.** `pkill -f "^python3\? tools/train"` 처럼 좁혀 쓸 것
- **재다운로드 불필요**: Drive 의 `best_accuracy`/`config.yml`. 크롭도 검출 작업엔 불필요
  (그래서 `test_every_crop_file_exists` 1건이 원격에서 실패하는 것은 **정상**)
- 런타임·터널 모두 수시로 죽는다. **체크포인트는 반드시 Drive 경로로** 저장할 것
...
| 4 | ~~검출 후처리 재스윕~~ | ✅ **소진** — 기본값이 최적. M1 그리드는 최적 구간(unclip 1.5)을 아예 안 훑었다 | `m3_det_finetune.md` §3-B |
...
| 10 | **학습/추론 스케일 정합** | 다음 세션 A(추론 확대)가 효과 없으면 재학습으로 | `m3_det_finetune.md` §4 |
| 11 | 운영 해상도 선택 | UI 후보 노출 수가 정해지면. 960/max 는 recall −4.5%p 로 precision +19.8%p | `m3_det_finetune.md` §1 |
| 12 | `tools/export_model.py` paddle 3.3 에서 파손 | 추론 서버 배포 시. 현재는 dygraph 로 우회 중 | `m3_det_finetune.md` §3 |
| 13 | **`margin` 을 val 에서 확정** | **다음 세션 C**. 지금 65.9%는 test 에서 고른 값이라 주장 불가 | `db_merge.md` §4 |
| 14 | **DB 유도 병합을 fine-tuned 검출기로 재측정** | **다음 세션 C**. 조각이 줄어(875→682) 이득이 달라진다 | `db_merge.md` §4 |
| 15 | 조건부 인식률 85.6% 가 낡음 | **다음 세션 C**. PP-OCRv5 박스로 잰 값이라 67.5% 추정의 근거가 약하다 | `m3_det_finetune.md` §5 |
