...
### 다음 단 하나의 행동 — **판단이 필요한 지점**

남은 수단은 전부 **비용이 크다.** M1/M2-B/M3 의 저비용 경로와 성격이 다르다.

1. **학습/추론 스케일 정합** — 640 크롭 학습 vs 2560 원본 추론. 학습 1회(~2시간)
2. **학습 데이터 증대** — 1,872장으로 +3.5%p 를 얻었다. 라벨링 비용
3. **M4 인식 fine-tuning 으로 전환** — 검출만으로 95% recall 을 채우기보다
   인식·매칭 쪽에서 격차를 나눠 지는 편이 현실적 (`metrics_policy.md` §5-B)

**권장: 3번.** 검출은 저비용 수단을 소진했고, 남은 16.5%p 를 검출 단독으로 메우려면
데이터 증대가 사실상 필수다. 반면 인식 쪽은 아직 fine-tuning 을 한 번도 하지 않았다.

**Colab 접속**: 셀 1 → 2 → 2-T 실행 후 터널 주소 전달.
Drive 의 `best_accuracy`/`config.yml` 을 그대로 쓰므로 사전학습 가중치·크롭 재생성은 불필요.
