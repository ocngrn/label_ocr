...
> ⚠️ **이 수치는 낙관 편향이다.** 평가 대상 318장에 train 이미지 75장이 섞여 있어 GT 의
> 23.1%가 학습 데이터다(곡면은 81.8%). 방향은 유지되나 크기는 과대평가이며,
> **곡면 24.2% 는 폐기해야 한다.** 상세 → [`reports/m3_det_finetune.md`](../reports/m3_det_finetune.md) 정정 절.
> 코드는 고쳤고(`extra_curved=False` 기본), 재측정은 아래 1번과 함께 1회로 처리한다.

### 다음 단 하나의 행동 — **코드 준비 완료, GPU 만 있으면 됨**
1. **← 여기부터**: **검출 후처리 재스윕** + **누수 없는 재측정**
   - `src/eval/det_sweep_trained.py` — 이미지당 순전파 1회로 60조합 평가
   - **val 에서 선택 → test 로 1회 확인** (test 에서 고르면 그 점수는 정직하지 않다)
   - 선택 규칙: `precision >= 56.5%`(현재 운영점) 하에서 plane recall 최대
   - M1 스윕은 사전학습 PP-OCRv5 분포에서 했다. 학습으로 분포가 바뀌었으므로
     (precision +23.5%p) 이번엔 개선 여지가 있다. **학습 불필요, GPU 약 30분.**
2. 그 다음: 학습/추론 스케일 정합 — 640 크롭으로 학습하고 2560 원본으로 추론 중이다

**필요한 사용자 조치**: Colab 셀 1 → 2 → 2-T 실행 후 터널 주소 전달.
Drive 의 `best_accuracy` 와 `config.yml` 을 그대로 쓰므로 사전학습 가중치·크롭 재생성은 불필요.
